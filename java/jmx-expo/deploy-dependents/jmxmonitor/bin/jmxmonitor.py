...
def jmx_update(srv, host, port):
    ...
    with LockFile(run_lock) as lock:
        cmd = "%s/bin/java -classpath .:../lib/dependency/*:../lib/jmxMonitor-0.0.1-SNAPSHOT.jar com.kali.sre.jmxmonitor.common.jmxCollClient %s %s %s %s" % (java_home, host, port, jmx_role, jmx_password)
...
if __name__ == '__main__':
    ...
    time.sleep(hash(args.port)/13 % 30)
